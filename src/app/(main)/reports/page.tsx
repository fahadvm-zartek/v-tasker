...
export default function ReportsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setReportsData(res);
      if (res && res.complaints && Array.isArray(res.complaints)) {
        const mapped = res.complaints.map((item: any) => {
          const initials = item.user?.full_name
            ? item.user.full_name.split(' ').map((n: string) => n[0]).join('').substring(0, 2).toUpperCase()
            : 'N/A';
          return {
            id: `#CMP-${item.id || 'N/A'}`,
            initials,
            avatarClass: 'bg-[#eef2ff] text-[#1B3061]',
            user: item.user?.full_name || item.user_name || 'N/A',
            category: item.category || 'General Report',
            severity: (item.severity || 'Medium') as Complaint['severity'],
            dateFiled: item.created_at ? new Date(item.created_at).toLocaleDateString() : 'N/A',
            assignedTo: item.assigned_to || 'Unassigned',
            status: (item.status || 'Pending') as Complaint['status'],
          };
        });
  ...
  const totalComplaints = reportsData?.total_complaints || complaintList.length || 156;
  const metrics: ReportMetric[] = [
    ...
    },
    {
      ...
      value: reportsData?.avg_resolution_time || '4.2h',
    ...
    },
    {
      ...
      value: reportsData?.critical_issues ? `${reportsData.critical_issues}` : '12',
      trend: 'high priority',
      trendTone: 'danger',
      icon: CircleAlert,
    ...
    },
    {
      ...
      value: reportsData?.satisfaction_score || '4.8/5',
      trend: '+2%',
      trendTone: 'positive',
      icon: SmilePlus,
  ...
  const defaultComplaints: Complaint[] = [
    { id: '#CMP-1042', initials: 'JS', avatarClass: 'bg-[#eef2ff] text-[#1B3061]', user: 'John Smith', category: 'App Issues', severity: 'Critical', dateFiled: 'Jun 28, 2025', assignedTo: 'Sarah Jenkins', status: 'Pending' },
    { id: '#CMP-1041', initials: 'AL', avatarClass: 'bg-[#cbf7e6] text-[#059669]', user: 'Alice Lee', category: 'Payment', severity: 'High', dateFiled: 'Jun 27, 2025', assignedTo: 'Mike Davis', status: 'Under Investigation' },
    { id: '#CMP-1040', initials: 'RW', avatarClass: 'bg-[#eedcff] text-[#8b5cf6]', user: 'Robert Wilson', category: 'Provider Behavior', severity: 'Medium', dateFiled: 'Jun 26, 2025', assignedTo: 'Emma Watson', status: 'Resolved' },
    { id: '#CMP-1039', initials: 'CD', avatarClass: 'bg-[#eef2f6] text-[#475569]', user: 'Claire Davis', category: 'Cancellation', severity: 'Low', dateFiled: 'Jun 25, 2025', assignedTo: 'Unassigned', status: 'Open' },
  ];

  const displayList = complaintList.length > 0 ? complaintList : defaultComplaints;
      ...
      <div className="animate-dashboard-entry space-y-6">
        ...
        <DashboardPanel>
          ...
          <div className="overflow-x-auto">
            <table className="ui-table min-w-[920px] table-fixed">
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan={8} className="py-8 text-center text-[12px] font-medium text-[#64748b]">
                      Loading reports & complaints...
                    </td>
                  </tr>
                ) : (
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-t border-[#e6ebf3] px-5 py-4">
            <p className="text-[13px] text-[#64748b]">Showing 1 to {filteredList.length} of {totalComplaints} complaints</p>
