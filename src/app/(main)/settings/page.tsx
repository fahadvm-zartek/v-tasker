...
const SettingsCard = ({ title, icon: Icon, children, footerAlign = 'end', onSave }: SettingsCardProps) => (
  <DashboardPanel>
    ...
    >
      ...
      <DashboardPrimaryButton onClick={onSave} className="h-10 rounded-[7px] px-6 text-[13px]">
...
const AdminProfileCard = () => {
  ...
  const initials = useMemo(() => {
    const source = `${profile.firstName || ''} ${profile.lastName || ''}`.trim() || profile.username;
  ...
  const displayName = `${profile.firstName || ''} ${profile.lastName || ''}`.trim() || profile.username || 'Admin User';
  ...
  return (
    <DashboardPanel className="p-6">
      <div className="flex items-start justify-between gap-4">
        <div className="flex min-w-0 items-center gap-4">
          ...
          <div className="min-w-0">
            ...
            <h2 className="mt-1 truncate text-[18px] font-bold leading-6 text-[#111827]">{displayName}</h2>
            <p className="mt-1 truncate text-[13px] font-medium leading-5 text-[#64748b]">{profile.email}</p>
          </div>
        </div>
      ...
      <div className="mt-5 border-t border-[#e4e9f1] pt-5">
        <div className="grid gap-3 rounded-[8px] bg-[#f8fafc] p-4">
          <div className="flex items-center justify-between gap-4">
            <span className="flex items-center gap-2 text-[13px] font-medium text-[#64748b]">
              <UserRound size={15} strokeWidth={2} />
              Username
            </span>
            <span className="truncate text-[13px] font-bold text-[#162033]">{profile.username || 'Not provided'}</span>
          </div>
          <div className="flex items-center justify-between gap-4">
            <span className="flex items-center gap-2 text-[13px] font-medium text-[#64748b]">
            ...
            <span className="text-[13px] font-bold text-[#162033]">{profile.role || 'ADMIN'}</span>
          </div>
          <div className="flex items-center justify-between gap-4">
            <span className="flex items-center gap-2 text-[13px] font-medium text-[#64748b]">
            ...
            <span className="truncate text-[13px] font-bold text-[#162033]">{profile.email}</span>
          </div>
        </div>
      </div>
    ...
    <DashboardPageShell>
      ...
      <div className="mt-6 grid gap-5 xl:grid-cols-[minmax(0,2fr)_minmax(300px,1fr)]">
        <div className="space-y-5">
          <SettingsCard title="Commission Settings" icon={Percent} onSave={handleSave}>
          ...
          <SettingsCard title="Cancellation Settings" icon={Clock3} onSave={handleSave}>
            <div className="grid gap-5 md:grid-cols-2">
              <SettingInput
