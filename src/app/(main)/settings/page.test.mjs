...
test('settings page includes admin profile and logout controls', async () => {
  ...
  assert.doesNotMatch(source, />\s*Phone\s*</);
  assert.doesNotMatch(source, /Pencil/);
  assert.doesNotMatch(source, /Edit mobile number/);
  assert.doesNotMatch(source, /phoneDraft/);
  assert.doesNotMatch(source, /handleSavePhone/);
  assert.doesNotMatch(source, /persistUserProfile\(\{\s*\.\.\.profile,\s*phone:/);
...
test('logout clears authenticated state, shows a success toast, and redirects to login', async () => {
  ...
  assert.match(logoutSource, /from '..\/..\/..\/services\/authService'/);
  assert.match(logoutSource, /const \{ AuthApiError, logout, clearAuthSession \} = authService/);
  assert.match(logoutSource, /await logout\(\)/);
  assert.match(logoutSource, /clearAuthSession\(\)/);
  assert.match(logoutSource, /error instanceof Error \? error\.message/);
  assert.match(logoutSource, /\[401, 403\]\.includes\(error\.status \?\? 0\)/);
