...
import { fetchDisputeById, resolveDispute, addDisputeActivity } from '../../../../services/disputeService';
...
export default function DisputeDetailPage({
  ...
  const activeTab: DisputeTab = tab === 'doer' ? 'doer' : 'poster';
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      const res = await fetchDisputeById(disputeId);
      if (res && res.raw) {
        setDisputeData(res.raw);
        if (res.raw.status || res.summary?.status) setStatus(res.raw.status || res.summary?.status);
        if (res.raw.activity_log || res.raw.activities) {
          setActivities(res.raw.activity_log || res.raw.activities || []);
        }
      } else {
        setDisputeData(null);
      }
  ...
  const handleResolve = async () => {
    ...
    try {
      ...
      setStatus('RESOLVED');
      await addDisputeActivity(disputeId, { comment: `Dispute marked as resolved: ${resolutionNotes || 'N/A'}` });
      await loadData();
    } catch (err: any) {
      alert(`Error resolving dispute: ${err?.message || 'Unknown error'}`);
    } finally {
  ...
  const handleAddActivity = async () => {
    ...
    try {
      await addDisputeActivity(disputeId, { comment: resolutionNotes });
      setResolutionNotes('');
      await loadData();
    } catch (err: any) {
  ...
  const raisedBy = disputeData?.raised_by?.full_name || disputeData?.poster_name || 'David L.';
  const raisedByEmail = disputeData?.raised_by?.email || disputeData?.poster_email || 'david.l@techsolutions.com';
  const against = disputeData?.against?.full_name || disputeData?.doer_name || 'Marcus G.';
  const againstEmail = disputeData?.against?.email || disputeData?.doer_email || 'marcus.g@example.com';
  const description = disputeData?.description || disputeData?.reason_details || (
    activeTab === 'poster'
      ? 'The deliverable provided does not meet the specifications outlined in the original statement of work. Multiple critical bugs were found in the final build, and the UI elements are misaligned across different screen sizes.'
      : 'The client is requesting additional work not specified in the original brief and has refused to release the milestone payment until these unquoted tasks are completed.'
  );
  const taskTitle = disputeData?.task?.title || disputeData?.task_title || `#TSK-${disputeId}`;
  const dateFiled = disputeData?.created_at ? new Date(disputeData.created_at).toLocaleString() : 'Oct 24, 2023 at 14:30 EST';
  const currentStatus = disputeData?.status || status;
  ...
  return (
    <DashboardPageShell contentClassName="px-5 pb-10 pt-5">
      {loading ? (
        <div className="p-8 text-center text-[12px] font-medium text-[#64748b]">Loading dispute details...</div>
      ) : null}
      ...
      <div className="animate-dashboard-entry grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div className="space-y-5">
          <DashboardPanel className="rounded-[8px] p-5">
            ...
            <div className="mt-5 flex items-center justify-between gap-3">
              ...
              <span className={dashboardStatusBadgeClass(currentStatus === 'RESOLVED' ? 'success' : 'warning')}>
                {currentStatus}
              </span>
            </div>
            ...
            <dl className="mt-5 grid gap-x-8 gap-y-5 sm:grid-cols-2">
              <div>
                ...
                <dd className="mt-1 text-[12px] font-bold text-[#0457cf]">{taskTitle}</dd>
              </div>
              <div>
                <dt className="text-[10px] font-bold uppercase tracking-[0.08em] text-[#72819a]">Date Filed</dt>
                <dd className="mt-1 text-[12px] font-semibold text-[#172033]">{dateFiled}</dd>
              </div>
              <div>
                <dt className="text-[10px] font-bold uppercase tracking-[0.08em] text-[#72819a]">Against (Task Poster)</dt>
                <dd className="mt-1 text-[12px] font-bold text-[#172033]">{against}</dd>
                <dd className="text-[11px] font-medium text-[#64748b]">{againstEmail}</dd>
              </div>
              <div>
                <dt className="text-[10px] font-bold uppercase tracking-[0.08em] text-[#72819a]">Raised By (Task Provider)</dt>
                <dd className="mt-1 text-[12px] font-bold text-[#172033]">{raisedBy}</dd>
                <dd className="text-[11px] font-medium text-[#64748b]">{raisedByEmail}</dd>
              </div>
              <div className="sm:col-span-2">
                <dt className="text-[10px] font-bold uppercase tracking-[0.08em] text-[#72819a]">Reason Category</dt>
                <dd className="mt-1 text-[12px] font-semibold text-[#172033]">{disputeData?.reason || 'Quality of Work'}</dd>
              </div>
              <div className="sm:col-span-2">
          ...
          <DashboardPanel className="rounded-[8px] p-5">
            ...
            <ol className="mt-5 space-y-4">
              {displayActivities.map((item: any, idx: number) => (
                <li key={idx} className="grid grid-cols-[20px_minmax(0,1fr)] gap-3">
                  <span className="relative flex justify-center">
                    <span className={cn('mt-1 h-3 w-3 rounded-full', item.active || idx === 0 ? 'bg-[#0457cf]' : 'bg-[#dbe4ef]')} />
                  </span>
                  <div>
                    <p className="text-[10px] font-medium text-[#64748b]">{item.time || item.created_at || 'N/A'}</p>
                    <p className="mt-1 text-[12px] font-medium text-[#172033]">{item.text || item.comment || item.activity || item.message}</p>
                  </div>
                </li>
              ))}
            </ol>
        ...
        <DashboardPanel className="h-fit rounded-[8px] p-5">
          ...
          <div className="mt-5 space-y-4">
            <label className="block">
              ...
              <select
                ...
                className="mt-1 h-10 w-full rounded-[6px] border border-[#dbe4ef] bg-white px-3 text-[12px] font-medium text-[#172033] outline-hidden focus:border-[#1B3061]"
              >
            ...
            {resolutionNotes ? (
              <button
                ...
                className="h-8 rounded-[6px] bg-[#1B3061] px-3 text-[11px] font-bold text-white"
              >
                Add Activity Log
              </button>
            ) : null}
            ...
            <div className="mt-3 space-y-3">
              <div className="flex items-center gap-3 rounded-[8px] border-2 border-[#60a5fa] bg-[#dbeafe] p-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[11px] font-medium text-[#475569]">{against}</p>
                </div>
              ...
              <div className="flex items-center gap-3 rounded-[8px] border border-[#dbe4ef] bg-white p-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[11px] font-medium text-[#475569]">{raisedBy}</p>
                </div>
                <Percent size={13} strokeWidth={2.2} className="text-[#334155]" />
          ...
          <div className="mt-5 space-y-4">
            <PayoutModal refundName={against} providerName={raisedBy} onConfirmPayout={handleResolve} />
            <button
              type="button"
              onClick={handleResolve}
              disabled={actionLoading}
              className={cn(
                "inline-flex h-10 w-full items-center justify-center gap-2 rounded-[6px] text-[12px] font-bold text-white transition-colors",
                currentStatus === 'RESOLVED' ? "bg-[#10b981]" : "bg-[#1B3061] hover:bg-[#142347]"
              )}
            >
              <CheckCircle2 size={14} strokeWidth={2.2} />
              {actionLoading ? 'Saving...' : currentStatus === 'RESOLVED' ? 'Resolved' : 'Mark as Resolved'}
            </button>
          </div>
        </DashboardPanel>
      </div>
