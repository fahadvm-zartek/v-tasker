...
import { updateRewardClaimStatus } from '../../../../../services/rewardsService';
...
export default function RewardClaimDetailPage({
  params,
}: {
  ...
  const [currentClaim, setCurrentClaim] = useState<ClaimDetail>(
    claimDetails[normalizedClaimId] ?? claimDetails['CLM-8824']
  );
  ...
  const handleUpdateStatus = async (newStatus: ClaimStatus) => {
    setSubmitting(true);
    try {
      await updateRewardClaimStatus(normalizedClaimId, newStatus);
      setCurrentClaim((prev) => ({ ...prev, status: newStatus }));
    } catch (err: any) {
      console.error('Failed to update status:', err);
      // Fallback update in UI
      setCurrentClaim((prev) => ({ ...prev, status: newStatus }));
    } finally {
      setSubmitting(false);
    }
  ...
  if (!isPending) {
    return (
      <DashboardPageShell contentClassName="flex min-h-[calc(100vh-76px)] items-center justify-center bg-[#7d8793] px-4 py-4">
          ...
          <header className="flex items-center justify-between gap-4 border-b border-[#e4eaf2] px-5 py-4">
            <div className="flex flex-wrap items-center gap-3">
              <h1 className="text-[18px] font-bold leading-6 text-[#202b3d]">
                Reward Fulfillment Detail - #{currentClaim.claimId}
              </h1>
              <span className={dashboardStatusBadgeClass(statusTone[currentClaim.status])}>{currentClaim.status}</span>
            </div>
            <Link
            ...
            <div className="space-y-4">
              <section className="rounded-[8px] border border-[#dbe4ef] bg-white p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-[16px] font-bold text-[#202b3d]">{currentClaim.reward}</h2>
                    <p className="mt-0.5 text-[11px] font-medium text-[#64748b]">{currentClaim.milestone}</p>
                  </div>
                </div>
                <dl className="mt-4 grid grid-cols-2 border-t border-[#dbe4ef] pt-3">
                  <div>
                    ...
                    <dd className="mt-1 text-[12px] font-semibold text-[#172033]">{currentClaim.value}</dd>
                  </div>
                  <div>
              ...
              <section className="rounded-[8px] border border-[#dbe4ef] bg-white p-4">
                ...
                <div className="mt-4 flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-[16px] font-bold text-[#202b3d]">{currentClaim.recipient}</p>
                    <p className="mt-0.5 text-[11px] font-medium text-[#64748b]">{currentClaim.account}</p>
                  </div>
                </div>
              ...
              <section className="rounded-[8px] border border-[#dbe4ef] bg-white p-4">
                ...
                <dl className="mt-3 space-y-3">
                  ...
                  <div>
                    ...
                    <dd className="mt-1 text-[12px] font-medium text-[#172033]">{currentClaim.method}</dd>
                  </div>
                  <div>
                    <dt className="text-[11px] font-medium text-[#8a98ad]">Tracking Number</dt>
                    <dd className="mt-1">
                      <button type="button" className="inline-flex items-center gap-1 text-[12px] font-bold text-[#0457cf]">
                        {currentClaim.tracking}
    ...
    <DashboardPageShell contentClassName="px-5 pb-10 pt-5">
      <div className="animate-dashboard-entry space-y-5">
        <nav className="flex items-center gap-2 text-[12px] font-medium text-[#64748b]" aria-label="Breadcrumb">
          ...
          <span aria-hidden="true">&gt;</span>
          <span>Claim #{currentClaim.claimId}</span>
        </nav>
          ...
          <DashboardPanel className="relative rounded-[8px] p-6">
            ...
            <dl className="relative mt-6 space-y-5">
              <div>
                ...
                <dd className="mt-2 text-[13px] font-semibold text-[#172033]">{currentClaim.recipient}</dd>
              </div>
              <div className="border-t border-[#edf1f7] pt-5">
                ...
                <dd className="mt-2 flex gap-2 text-[13px] font-medium leading-5 text-[#334155]">
                  ...
                  <span>{currentClaim.address.join('\n')}</span>
                </dd>
              </div>
              <div className="border-t border-[#edf1f7] pt-5">
              ...
              <div className="rounded-[6px] bg-[#eef3ff] p-4">
                ...
                <dd className="mt-2 text-[13px] font-bold text-[#0457cf]">{currentClaim.reward}</dd>
              </div>
            </dl>
          </DashboardPanel>
          ...
          <DashboardPanel className="rounded-[8px] p-6">
            ...
            <div className="mt-6 grid gap-5 sm:grid-cols-2">
              <Field label="Shipment Provider" required>
                <input
                  type="text"
                  defaultValue={currentClaim.provider}
