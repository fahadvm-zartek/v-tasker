...
test('user details page loads the selected user from api/users/{id}', async () => {
  const source = await readDetailsPage();

  assert.match(source, /useParams/);
  assert.match(source, /fetchUserById/);
  assert.match(source, /useEffect/);
  assert.match(source, /setUserDetail/);
  assert.match(source, /params\?\.id/);
});

test('user details page renders the selected user profile summary from api data with N/A fallbacks', async () => {
  const source = await readDetailsPage();

  for (const text of [
    'getIdentityInfoItems',
    'getContactRows',
    'getOverviewMetrics',
    'EMPTY_VALUE',
    'userSummary?.name ?? EMPTY_VALUE',
    "userSummary?.initials ?? 'NA'",
    'userSummary?.role ?? EMPTY_VALUE',
  ...
  assert.doesNotMatch(profileOverviewBlock, /Sarah Mitchell/);
  assert.doesNotMatch(profileOverviewBlock, /S\.Mitchell/);
  assert.doesNotMatch(profileOverviewBlock, /CST-9824/);
  assert.doesNotMatch(profileOverviewBlock, /Jan 12, 2024/);
  assert.doesNotMatch(profileOverviewBlock, /Oct 24, 2026/);
});

test('user details page maps nested backend user profile fields', async () => {
  const source = await readDetailsPage();

  for (const text of [
    'getNestedDetailText',
    'getNestedDetailNumber',
    "'user_type.name'",
    "'profile.bio'",
    "'profile.suburb_display'",
    "'profile.tasks_completed'",
    "'profile.tasks_posted'",
...
test('user details profile overview uses the premium corporate component structure', async () => {
  ...
  for (const text of [
    ...
    'getIdentityInfoItems(userDetail, userSummary)',
  ]) {
    assert.match(source, new RegExp(text.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
  }

  assert.match(source, /<ProfileOverview\s+userDetail=\{userDetail\}\s+userSummary=\{userSummary\}\s*\/>/);
...
test('user details identity information is arranged as four columns and two rows', async () => {
  const source = await readDetailsPage();
  const identityInfoItems = extractBlock(source, 'const getIdentityInfoItems', 'const AccountStanding');

  for (const text of ['Name', 'Member ID', 'Milestone', 'Student Verification Expiry']) {
    assert.match(identityInfoItems, new RegExp(text.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
  }

  for (const text of ['Joining Date', 'Role', 'Status', 'Interaction Date']) {
    assert.match(identityInfoItems, new RegExp(text.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
  }
...
test('user details page includes the user metric statistics row only', async () => {
  const source = await readDetailsPage();
  const statisticsBlock = extractBlock(source, 'const getOverviewMetrics', 'const tabs');
