...
export const DashboardPagination = ({
  ...
  <nav className="flex items-center gap-2" aria-label={label}>
    <button
      type="button"
      className={cn('text-[#94a3b8]', isPreviousDisabled && 'cursor-not-allowed text-[#cbd5e1]')}
    ...
    {pages.map((page, pageIndex) => (
      <button
        ...
        type="button"
        disabled={page === '...'}
        {...(onPageChange && page !== '...' ? { onClick: () => onPageChange(page) } : {})}
        className={cn(
    ...
    <button
      type="button"
      className={cn('text-[#94a3b8]', isNextDisabled && 'cursor-not-allowed text-[#cbd5e1]')}
