...
test('dashboard placeholder uses the compact layout with a fixed sidebar offset', async () => {
  const [pageSource, shellSource, sidebarSource, headerSource] = await Promise.all([
    readFile(new URL('../app/(main)/page.tsx', import.meta.url), 'utf8'),
    readFile(new URL('./dashboard-ui.tsx', import.meta.url), 'utf8'),
  ...
  assert.match(shellSource, /<Sidebar\s*\/>/);
...
test('dashboard top chrome matches the reference screenshot sizing', async () => {
  const [shellSource, sidebarSource, headerSource] = await Promise.all([
    readFile(new URL('./dashboard-ui.tsx', import.meta.url), 'utf8'),
    readFile(new URL('./Sidebar.tsx', import.meta.url), 'utf8'),
    readFile(new URL('./Header.tsx', import.meta.url), 'utf8'),
  ]);

  assert.match(shellSource, /bg-\[#f7f8fa\]/);
  assert.match(shellSource, /pl-\[var\(--layout-sidebar-current\)\]/);
  assert.match(shellSource, /px-8/);
  assert.match(sidebarSource, /w-\[var\(--layout-sidebar-current\)\]/);
...
test('header profile avatar links to settings', async () => {
  const headerSource = await readFile(new URL('./Header.tsx', import.meta.url), 'utf8');

  assert.match(headerSource, /from 'next\/link'/);
  assert.match(headerSource, /href="\/settings"/);
  assert.match(headerSource, /aria-label="Open settings"/);
  assert.doesNotMatch(headerSource, /<div className="flex h-9 w-9 items-center justify-center rounded-full bg-\[#1B3061\]/);
});
...
test('sidebar toggle collapses the sidebar and updates the page content offset', async () => {
  const [sidebarSource, shellSource, placeholderSource] = await Promise.all([
    readFile(new URL('./Sidebar.tsx', import.meta.url), 'utf8'),
    readFile(new URL('./dashboard-ui.tsx', import.meta.url), 'utf8'),
  ...
  assert.match(shellSource, /pl-\[var\(--layout-sidebar-current\)\]/);
  assert.match(placeholderSource, /DashboardPageShell/);
});
...
test('locked overlay covers only the main content while keeping sidebar untouched', async () => {
  const [placeholderSource, shellSource, lockedOverlaySource] = await Promise.all([
    readFile(new URL('./DashboardPlaceholderPage.tsx', import.meta.url), 'utf8'),
    readFile(new URL('./dashboard-ui.tsx', import.meta.url), 'utf8'),
    readFile(new URL('./LockedOverlay.tsx', import.meta.url), 'utf8'),
  ]);

  assert.match(shellSource, /<Sidebar\s*\/>[\s\S]*<main className="[^"]*pl-\[var\(--layout-sidebar-current\)\][^"]*"/);
  assert.match(shellSource, /<Header\s*\/>[\s\S]*<div className=\{cn\('dashboard-container/);
  assert.match(placeholderSource, /<section className="relative min-h-\[calc\(100vh-134px\)\]" aria-label=\{\`\$\{title\} coming soon\`\}>[\s\S]*<LockedOverlay\s*\/>[\s\S]*<\/section>/);
