...
test('app toast uses the compact premium success toast structure and styling', async () => {
  ...
  assert.match(source, /<Icon size=\{17\}/);
...
test('app toast supports an error variant for failed auth messages', async () => {
  const source = await readSource('./AppToast.tsx');

  assert.match(source, /variant\?: 'success' \| 'error'/);
  assert.match(source, /AlertCircle/);
  assert.match(source, /variant = 'success'/);
  assert.match(source, /toastVariants/);
  assert.match(source, /border-\[#fecaca\]/);
  assert.match(source, /bg-\[#fef2f2\]/);
  assert.match(source, /bg-\[#dc2626\]/);
  assert.match(source, /focus-visible:ring-\[#dc2626\]\/20/);
});
