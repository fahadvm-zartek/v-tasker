...
test('login validates email and password live and prevents invalid submission', async () => {
  ...
  assert.doesNotMatch(cardSource, /validateUsername|Username is required|handleUsername|usernameFieldState|autoComplete="username"/);
});
...
test('successful login shows a success toast and redirects to dashboard', async () => {
  ...
  assert.match(cardSource, /setToastTitle\('Signed in'\)/);
  assert.match(cardSource, /setToastMessage\('Welcome back, Admin User\.'\)/);
  assert.match(cardSource, /title=\{toastTitle\}/);
  assert.match(cardSource, /message=\{toastMessage\}/);
  ...
  assert.match(cardSource, /router\.push\('\/dashboard'\)/);
...
test('successful login submits credentials through the auth service', async () => {
  const cardSource = await readSource('./login/LoginCard.tsx');

  assert.match(cardSource, /from '..\/..\/services\/authService'/);
  assert.match(cardSource, /await login\(\{ email, password \}\)/);
  assert.match(cardSource, /persistAuthSession\(session\)/);
  assert.doesNotMatch(cardSource, /await login\(\{ username, email, password \}\)/);
  assert.doesNotMatch(cardSource, /window\.localStorage\.setItem\('v-tasker-authenticated', 'true'\)/);
});

test('login displays backend field errors next to the matching fields', async () => {
  const cardSource = await readSource('./login/LoginCard.tsx');

  assert.match(cardSource, /AuthApiError/);
...
test('forgot password route uses the shared auth background and reset card', async () => {
  const [pageSource, indexSource] = await Promise.all([
    readSource('../app/(auth)/forgot-password/page.tsx'),
    readSource('./login/index.ts'),
  ]);
...
test('register route uses the shared auth background and register card', async () => {
  const [pageSource, indexSource] = await Promise.all([
    readSource('../app/(auth)/register/page.tsx'),
    readSource('./login/index.ts'),
  ]);
...
test('register card captures account details and submits through the auth service', async () => {
  const cardSource = await readSource('./login/RegisterCard.tsx');

  assert.match(cardSource, /'use client'/);
  assert.match(cardSource, /Create Account/);
  assert.match(cardSource, /Already have an account\?/);
  assert.match(cardSource, /href="\/login"/);
  assert.match(cardSource, /First Name/);
  assert.match(cardSource, /Last Name/);
  assert.match(cardSource, /Email Address/);
  assert.match(cardSource, /Password/);
  assert.match(cardSource, /Confirm Password/);
  assert.match(cardSource, /from '..\/..\/services\/authService'/);
  assert.match(cardSource, /const \{ AuthApiError, register, persistAuthSession \} = authService/);
  assert.match(cardSource, /first_name: firstName/);
  assert.match(cardSource, /last_name: lastName/);
  assert.match(cardSource, /user_type_name: 'ADMIN'/);
  assert.match(cardSource, /await register\(\{\s*email,\s*password1: password,\s*password2: confirmPassword,\s*first_name: firstName,\s*last_name: lastName,\s*user_type_name: 'ADMIN',\s*\}\)/);
  assert.match(cardSource, /persistAuthSession\(session\)/);
  assert.match(cardSource, /router\.push\('\/dashboard'\)/);
  assert.doesNotMatch(cardSource, /Username|register-username|autoComplete="username"|await register\(\{ username,/);
});

test('register displays backend field errors next to the matching fields', async () => {
  const cardSource = await readSource('./login/RegisterCard.tsx');

  assert.match(cardSource, /AuthApiError/);
  assert.match(cardSource, /apiFieldErrors/);
  assert.match(cardSource, /getFieldError\(error, 'first_name'\)/);
  assert.match(cardSource, /getFieldError\(error, 'last_name'\)/);
  assert.match(cardSource, /getFieldError\(error, 'email'\)/);
  assert.match(cardSource, /getFieldError\(error, 'password1'\)/);
  assert.match(cardSource, /getFieldError\(error, 'password2'\)/);
...
test('register card validates names, email, and matching passwords before submission', async () => {
  const cardSource = await readSource('./login/RegisterCard.tsx');

  assert.match(cardSource, /validateRegisterName/);
  assert.match(cardSource, /First name is required\./);
  assert.match(cardSource, /Last name is required\./);
  assert.match(cardSource, /onInput=\{handleFirstNameInput\}/);
  assert.match(cardSource, /onInput=\{handleLastNameInput\}/);
  assert.match(cardSource, /validateRegisterEmail/);
  assert.match(cardSource, /Email is required\./);
  assert.match(cardSource, /Please enter a valid email address\./);
  assert.match(cardSource, /Password is required\./);
  assert.match(cardSource, /Password must be at least 8 characters\./);
  assert.match(cardSource, /Passwords must match\./);
  assert.match(cardSource, /onInput=\{handleEmailInput\}/);
  assert.match(cardSource, /onInput=\{handlePasswordInput\}/);
  assert.match(cardSource, /onInput=\{handleConfirmPasswordInput\}/);
  assert.match(cardSource, /disabled=\{!isFormValid \|\| isSubmitting\}/);
  assert.match(cardSource, /event\.preventDefault\(\)/);
  assert.doesNotMatch(cardSource, /validateRegisterUsername|Username is required|handleUsernameInput/);
});

test('login and register pages link to each other', async () => {
  const [loginCardSource, registerCardSource] = await Promise.all([
    readSource('./login/LoginCard.tsx'),
    readSource('./login/RegisterCard.tsx'),
  ]);

  assert.match(loginCardSource, /href="\/register"/);
  assert.match(loginCardSource, /Create account/);
  assert.match(registerCardSource, /href="\/login"/);
  assert.match(registerCardSource, /Sign in/);
});
