...
const PanelHeader = ({ title }: { title: string }) => (
  <div className="flex h-[42px] items-center justify-between px-4">
    ...
    <Link href="/service-categories" className="text-[13px] font-bold text-[#0b63ce]">
      View All
    </Link>
  </div>
);
...
const OverviewDashboard = () => {
  ...
  const dynamicMetricCards: MetricCard[] = React.useMemo(() => {
    ...
    return [
      {
        title: 'Tasks Posted',
        value: tasks.total !== undefined ? String(tasks.total) : '1,284',
        icon: Smartphone,
        tone: 'info',
      },
      {
        title: 'Completed Tasks',
        value: tasks.completed !== undefined ? String(tasks.completed) : '946',
        icon: CheckCircle2,
        tone: 'success',
      },
      {
        title: 'Total Payments',
        value: payments.total_amount !== undefined ? `$${payments.total_amount}` : '$2,48,650',
        icon: IndianRupee,
        tone: 'success',
      },
      {
        title: 'Total Commission',
        value: payments.total_commission !== undefined ? `$${payments.total_commission}` : '$24,865',
        icon: BadgePercent,
        tone: 'success',
        detail:
          payments.normal_commission && payments.student_commission
            ? `Normal Users: $${payments.normal_commission} Student Users: $${payments.student_commission}`
            : 'Normal Users: $18,815 Student Users: $6,050',
      },
      {
        title: 'Task Deletion Rate',
        value: tasks.deletion_rate !== undefined ? `${tasks.deletion_rate}%` : '6.8%',
        icon: Trash2,
        tone: 'danger',
        detail: tasks.deletions ? `${tasks.deletions} deletions` : '54 deletions',
      },
      {
        title: 'Users Requiring Review',
        value: users.pending_approvals !== undefined ? String(users.pending_approvals) : '18',
        icon: ShieldAlert,
        tone: 'danger',
        action: 'Review now',
      },
      {
        title: 'Tasks Without Offers',
        value: tasks.no_offers !== undefined ? String(tasks.no_offers) : '142',
        icon: CircleAlert,
        tone: 'warning',
      },
      {
        title: 'Cancellation Rate',
        value: tasks.cancellation_rate !== undefined ? `${tasks.cancellation_rate}%` : '6.8%',
        icon: Ban,
        tone: 'warning',
        detail: tasks.cancellations ? `${tasks.cancellations} cancellations` : '87 cancellations',
      },
      {
        title: 'Cancellations Earnings',
        value: payments.cancellations_earnings !== undefined ? `$${payments.cancellations_earnings}` : '$3,210',
        icon: CreditCard,
        tone: 'info',
        detail: tasks.cancellations ? `${tasks.cancellations} cancellations` : '87 cancellations',
      },
      {
        title: 'Suspended & Reported',
        value: users.suspended !== undefined ? String(users.suspended) : '18',
        icon: AlertTriangle,
        tone: 'danger',
        detail: users.reported ? `Reported ${users.reported} Suspended Users` : 'Reported 7 Suspended Users',
        action: 'View flagged users',
      },
      {
        title: 'Moderations Detected',
        value: dashboardData?.moderations?.detected !== undefined ? String(dashboardData.moderations.detected) : '356',
        icon: MessageSquareWarning,
        tone: 'info',
        detail: 'auto-scanned',
        action: '+12 detected today',
      },
      {
        title: 'Hard Moderations',
        value: dashboardData?.moderations?.hard !== undefined ? String(dashboardData.moderations.hard) : '61',
        icon: Gavel,
        tone: 'danger',
        action: 'Review now',
      },
      {
        title: 'Milestone Products Total',
        value: dashboardData?.rewards?.products_total !== undefined ? String(dashboardData.rewards.products_total) : '1,248',
        icon: Trophy,
        tone: 'warning',
        detail: 'Across all reward tiers',
      },
      {
        title: 'Total Points Granted',
        value: dashboardData?.rewards?.points_granted !== undefined ? String(dashboardData.rewards.points_granted) : '48,500',
        icon: Gift,
        tone: 'warning',
        detail: 'Equiv: $485.00 AUD',
      },
      {
        title: 'Disputes',
        value: disputes.open !== undefined ? String(disputes.open) : disputes.total !== undefined ? String(disputes.total) : '7',
        icon: Flag,
        tone: 'danger',
        detail: disputes.closed !== undefined ? `Open Dispute ${disputes.closed} Closed Dispute` : 'Open Dispute 18 Closed Dispute',
      },
    ];
  }, [dashboardData]);
