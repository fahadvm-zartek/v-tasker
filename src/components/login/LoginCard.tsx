...
const { AuthApiError, login, persistAuthSession } = authService;
...
const LoginCard = () => {
  ...
  const passwordMessage = touchedFields.password ? passwordError || apiFieldErrors.password : '';
  ...
  return (
    <>
      ...
      <div className="mt-6 flex justify-center border-t border-[#ece8e2] pt-5 text-[10px] text-[#5f5b56]">
        <span>New here?</span>
        <Link href="/register" className="ml-1 font-semibold text-[#1B3061] transition-colors hover:text-[#E68A2E]">
          Create account
        </Link>
      </div>
      </section>
